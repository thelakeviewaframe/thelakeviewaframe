...
// Creates a Stripe Checkout session (card authorized but NOT captured, see
// capture_method: 'manual' below) and a 'pending' booking row with a random
// review_token. Dates are NOT blocked yet, that happens in the webhook once
// the card authorization succeeds, so an abandoned checkout doesn't lock out
// other guests. The guest's card is only ever charged if the host approves
// the request via the emailed approve link (see app/api/booking/respond).
export async function POST(request) {
  ...
  const session = await stripe.checkout.sessions.create({
        ...
        // Authorize the card only, funds are reserved but not charged until the
        // host approves the request (or released if declined).
